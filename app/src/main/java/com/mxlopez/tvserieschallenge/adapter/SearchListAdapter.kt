...
class SearchListAdapter(private val list: List<SearchedShow>?): RecyclerView.Adapter<SearchListAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.seriesName.text = list?.get(position)!!.show?.name ?: "N/A"
        holder.cellBody.setOnClickListener {
            Toast.makeText(ctx, "${list[position].show?.name}", Toast.LENGTH_SHORT).show()
        }
    }
